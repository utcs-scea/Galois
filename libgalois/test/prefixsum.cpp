...
uint64_t transmute (const std::pair<uint64_t, uint64_t>& p) {return p.second - p.first;};
uint64_t scan_op   (const std::pair<uint64_t, uint64_t>& p, const uint64_t& l) {return p.second - p.first + l;};
...
{ galois::SharedMemSys Galois_runtime;
  ...
  std::cout << "Host" << "," << "Lock Name" << "," << "numThreads" << "," << "Time (us)" << std::endl;

  //using PTS   = PerThreadStorage<unsigned>;

  auto src = (std::pair<uint64_t, uint64_t>*) malloc(sizeof(std::pair<uint64_t, uint64_t>) * (1<<30));
  auto dst = (uint64_t*) malloc(sizeof(uint64_t)*(1<<30));

  for(uint64_t i = 0; i < (1<<30); i ++) src[i] = {0, 1};

  using PSUM = PrefixSum<std::pair<uint64_t, uint64_t>, uint64_t,
    transmute, scan_op, combiner,
    CacheLinePaddedArr>;
