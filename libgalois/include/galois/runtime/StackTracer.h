...
public:
  ...
  MPSCBuffer(uint16_t lim, volatile uintptr_t* buf) : head(0), buf(buf), LIMIT(lim) {}
  */
  ...
  uint16_t get_size()
  {
    ...
    if(head_local > LIMIT) return LIMIT;
    else return head_local;
  }
...
struct StackCap
{
  ...
  uint64_t  getStackPtr() { uint64_t a; return (uint64_t)&a;}
...
public:
  ...
  */
  ...
  void capture_stack_info()
  {
    ...
    if(change_top) __atomic_store_8(&cap.top, curr, __ATOMIC_RELAXED);
  ...
  uint64_t get_max()
  {
    ...
    for(uint16_t i = 0; i < caps.get_size(); i++)
    {
      ...
      bool valid_vals = (cont_cap->top != init_top) && (cont_cap->bot != init_bot);
  ...
  int get_top_bot(uint16_t idx, uint64_t& top, uint64_t& bot)
  {
    ...
    ThreadStackCap* cont_cap = (ThreadStackCap*) tr_cap_addr;
    if(ret != 0 || cont_cap == nullptr) return -1;
    top = cont_cap->top;
    bot = cont_cap->bot;
    return 0;
  }
...
extern "C"
{
  void __cyg_profile_func_enter(void *this_fn, void* call_site) {cyg_profile_func_stack(this_fn, call_site);}
  void __cyg_profile_func_exit(void *this_fn, void* call_site)  {cyg_profile_func_stack(this_fn, call_site);}
}
